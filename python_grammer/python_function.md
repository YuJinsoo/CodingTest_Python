...
- 프로그래밍 언어에서 FirstClass Object(또는 First Class Citizen)는 해당 언어 내에서 일반저긍로 다른 모든 개체에
  <br>통용가능한 동작(operation)이 지원되는 개체(entity)를 의미합니다.

- 아래 세 가지 조건을 만족해야 1급 객체입니다.

  > 1. 함수의 인자로 전달되거나,
  > 2. 함수의 반환값이 된거나,
  > 3. 수정되고 할당되는 것들을 전제로 합니다

- 파이썬의 `list`, `str` `int`등과 같은 자료형은 함수의 인자로 전달되거나, 반환값이 되거나, 수정되고 할당할 수 있습니다.

  - 이러한 자료형들은 1급객체

- C언어에서 함수 포인터로 함수를 넘겨줄 수 있는데, 이것은 포인터가 1급객체이지 함수가 1급객체라는 표현은 적절하지 않습니다.

```python
# python에서 함수는 1급객체입니다.
def add(a, b):
    return a + b

def execute(func, *args):
    return func(*args) # 조건 2.

f = add          # 조건 3.
execute(f, 3, 5) # 조건 1.
```

> 1. f 라는 함수가 execute 의 함수의 인자로 전달되었고,
> 2. 함수 내부에서 인자로 받은 함수 func 를 문제없이 사용하고 있으며,
> 3. add 라는 원 함수의 이름이 마음에 안 들었는지 f 라는 새 이름에 할당했다.
